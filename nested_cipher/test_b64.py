...
class Test_b64:
    ...
    def __init__(self) -> None:
        self.__result: NamedTuple = NamedTuple('result', (('passed', bool), ('name', str), ('encoded', bytes), ('decoded', bytes), ('time', str)))
    ...
    def _ab64(self, data: bytes) -> bool:
        ...
        return self.__result(passed, name, enc, dec, times)
...
if __name__ == '__main__':
    ...
    import json
    result_file = _path.realpath('test_file/test_nested_b64.jsn')
    ...
    with open(result_file, 'w') as f:
        ...
        for it in test_data:
            t = tester.test(it)
            val_res = ({'NAME':str(j.name),'PASSED':str(j.passed),'TIME':str(j.time),'ENCODED':j.encoded.decode('ascii')} for j in t[1])
